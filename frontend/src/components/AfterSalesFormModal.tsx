...
import { X, Plus, Trash2, Package, Search, User, Phone, Car, AlertCircle } from 'lucide-react'
import type {
  AfterSalesType, AfterSalesPriority, IssueCategory, Order, AfterSalesPartItem
} from '@/types'
import {
...
export default function AfterSalesFormModal({ isOpen, onClose, orderId }: Props) {
  const { fetchOrders, orders, createAfterSales, currentUser } = useStore()
  ...
  const handleSelectOrder = (order: Order) => {
    ...
    const partsCost = parts.reduce((sum, p) => sum + p.unitPrice * p.quantity, 0)
    setFormData((prev) => ({
      ...prev,
      partsCost,
      customerCharge: partsCost + prev.laborFee,
    }))
  }
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-carbon-800 rounded-xl border border-carbon-500/30 w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div>
            <label className="text-sm text-moto-silver font-medium mb-2 block">关联配件</label>
            {selectedParts.length === 0 ? (
              ...
              <div className="space-y-2">
                {selectedParts.map((part, idx) => (
                  <div key={idx} className="flex items-center gap-3 p-3 bg-carbon-700/50 rounded-lg">
                    <img
                      src={part.partImage}
                      alt={part.partName}
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  setFormData({
                    ...formData,
                    laborFee: v,
                    customerCharge: v + formData.partsCost - formData.warrantyCoverage,
                  })
                }}
                className="w-full bg-carbon-700 text-moto-silver text-sm rounded-lg px-3 py-2 border border-carbon-500/30 focus:outline-none focus:border-moto-orange/50"
                min={0}
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  setFormData({
                    ...formData,
                    partsCost: v,
                    customerCharge: formData.laborFee + v - formData.warrantyCoverage,
                  })
                }}
                className="w-full bg-carbon-700 text-moto-silver text-sm rounded-lg px-3 py-2 border border-carbon-500/30 focus:outline-none focus:border-moto-orange/50"
                min={0}
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  setFormData({
                    ...formData,
                    warrantyCoverage: v,
                    customerCharge: formData.laborFee + formData.partsCost - v,
                  })
                }}
                className="w-full bg-carbon-700 text-moto-silver text-sm rounded-lg px-3 py-2 border border-carbon-500/30 focus:outline-none focus:border-moto-orange/50"
                min={0}
